...
class MagazineServiceTest {
    ...
    void getListAuthor_ShouldThrowException_WhenAuthorNotExists() throws Exception {
        CreateMagazineDTO createMagazineDTO1 = CreateMagazineDTO.builder().title("Teste").authorIdList(authorIdList).publishedDate(LocalDate.now()).tipologyMagazine(TipologyMagazine.DAILY).build();

        when(createMagazineDTO.getAuthorIdList()).thenReturn(authorIdList);
